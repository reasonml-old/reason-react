...
module SyntheticEvent = {
  ...
  module Impl (Type : Type) => {
    type t_syntheticEvent = Type.t;

    external bubbles : t_syntheticEvent => bool = "" [@@bs.get];
    external cancelable : t_syntheticEvent => bool = "" [@@bs.get];
    external currentTarget : t_syntheticEvent => ReasonJs.Document.element = "" [@@bs.get]; /* Should return Dom.evetTarget */
    ...
    external nativeEvent : t_syntheticEvent => Js.t {..} = "" [@@bs.get]; /* Should return Dom.event */
    ...
    external target : t_syntheticEvent => ReasonJs.Document.element = "" [@@bs.get]; /* Should return Dom.evetTarget */
  ...
  include Impl { type t = syntheticEvent };
};

module ClipboardEvent = {
  type t = clipboardEvent;

  include SyntheticEvent.Impl { type nonrec t = t };
